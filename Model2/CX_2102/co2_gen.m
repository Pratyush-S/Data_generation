...
for p=1:Tc
    ...
    for i=((p-1)*60+1):(p*60)
        for j=1:6
            ...
            temp2=Dc{j};
            temp=[temp1 temp2];      
            [vtr1,vtr2,pc]=Recirculation(temp,factor_T(i),bias(i,j),temp_var_cc_recirc,temp_var_co_recirc,pc);
    ...
    simulink_values{5}(end+1,1)= (factor_T((p-1)*60+1))*1.2;
...
%% CO2 perminute samplng file store etc.
    ...
    Temperature_sim = simulink_values{1};
    Pressure_sim = simulink_values{2};
    Humidity_sim = simulink_values{3};
    Ozone_ref = simulink_values{4};
    Flowrate_sim = simulink_values{5};
...
k=0;
    for j=1:(Tc)
        ...
        flightNo2(k)="JAI808";
        pas_cnt(k) = P;
        t_sim(k) = Temperature_sim(k);
        p_sim(k) = Pressure_sim(k);
        h_sim(k) = Humidity_sim(k);
    end
...
% ad = length(flightNo2)
% ae = length(t_sim)
XY = table(Time2,CO2min_Zone,flightNo2,t_sim,p_sim,h_sim,tailNo2,pas_cnt);
fileName2 = 'Simulink_Data2.xlsx';
% fileName_template='Simulink_Data_Template.xlsx';
% copyfile(fileName_template,fileName)
writetable(XY,fileName2,'sheet',1);
