...
%%s%%
temp_var_cc_recirc = 0.0003;
temp_var_co_recirc = 0.21;
%%s%%
...
%     [T, Pcabin, H, O3] = getsimulinkdata(Foz, Teq, (factor_T( ((p-1)*60)+1) *1.2) );
    for i=((p-1)*60+1):(p*60)
        for j=1:6
            D0 = (AA{i}.*A);
            D1 = (AC{i}.*C);
            ...
            temp2=Dc{j};
            temp=[temp1 temp2];      
            [vtr1,vtr2,pc]=Recirculation2(temp,factor_T(i),bias(i,j),temp_var_cc_recirc,temp_var_co_recirc,pc);
            bufData(j,1)=vtr1*1000000;
            conc_co2{j}(end+1,1)=vtr1;
            conc_o2{j}(end+1,1)=vtr2;
        end
        temp_var_cc_recirc =( 5*( conc_co2{1}(end,1) + conc_co2{2}(end,1) + conc_co2{3}(end,1) + conc_co2{4}(end,1) +conc_co2{5}(end,1) ) + (6*conc_co2{6}(end,1)))/31;
        temp_var_co_recirc =( 5*( conc_o2{1}(end,1) + conc_o2{2}(end,1) + conc_o2{3}(end,1) + conc_o2{4}(end,1) +conc_o2{5}(end,1) ) + (6*conc_o2{6}(end,1)))/31;
    end
    for q=1:6
...
%     simulink_values{2}(end+1,1)= Pcabin;
...
X = table(Time,CO2_Zone,flightNo,tailNo,pas_cnt); %Binding Time with Data
...
% %X = table(Time,CO2_Zone,Markers,flightNo,tailNo); %Binding Time with Data
% delete(simulink_values{1}(1))
% delete(simulink_values{2}(1))
% delete(simulink_values{3}(1))
% delete(simulink_values{4}(1))
% delete(simulink_values{5}(1))
% Temp_simulink = simulink_values{1}
aa = length(Time2)
ab = length(CO2min_Zone)
ac = length(tailNo2)
ad = length(flightNo2)
% ae = length(simulink_values)
XY = table(Time2,CO2min_Zone,flightNo2,tailNo2,pas_cnt2);
...
% XYZ = table(Time2,CO2min_Zone,flightNo2,tailNo2,pas_cnt,Temp_simulink); %Binding Time with Data
% writetable(XYZ,fileName,'sheet',6);
%winopen(fileName)
...
function [new_cc,new_co,pc] = Recirculation2(Mat,ff,offset,cc,co,pc)
    co_const_bleed = 0.21;
    cc_const_bleed = 0.0003;
    co_const_recirc = 0.2086;
    cc_const_recirc = 0.0016;
